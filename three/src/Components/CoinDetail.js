...
const CoinDetail = () => {
  ...
  useEffect(() => {
    const fetchCoin = async () => {
      try {
        const { data } = await axios.get(`${server}/coins/${parmes.id}`);

        const { data: chartData } = await axios.get(
          `${server}/coins/${parmes.id}/market_chart?vs_currency=${currency}&days=${days}`
        );
      ...
      {loading ? (
        ...
        <>
          <Box borderWidth={1} w={"full"}>
            ...
            <Chart arr={chartArrays} currency={currencySymbol} days={days} />
          </Box>
          ...
          <VStack spacing={"4"} p={"16"} alignItems={"flex-start"}>
            ...
            <Badge fontSize={"2xl"} bg={"blackAlpha.900"} color={"white"}>
